...
class FittsTestUI extends UIClass {
    ...
    override configure(newState : string) {
        ...
        switch (this.currentState) {
            case 'start':
                ...
                //Set the target and background visibility
                this.theTarget.visible = false;
                this.theBackground.visible = true;

                //Set the visuals to be redrawn
                this.theBackground.declareDamaged();
                this.theTarget.declareDamaged();
                this.theReticle.declareDamaged();

            break;
            case 'begin_trial':
                
                //Set a new background message
                this.theBackground.msg1 = "Trial #" + this.trialCount + " of 10";
                this.theBackground.msg2 = "";
                this.theBackground.msg3 = "";

                //Show the reticle and hide the target
            ...
            case 'in_trial':
                ...
                //Show the target and hide the reticle
                this.theReticle.visible = false;
    ...
    newTrial() {
        ...
        if (this.trialCount > this.MAX_TRIALS) {
            ...
            // === YOUR CODE HERE ===
            // Set the new locations for the reticle and target
...
class Target extends ScreenObject{
    ...
    // optional diameter.  If diameter is not included the size is left unchanged.
    ...
    newGeom(newCentX : number, newCentY : number, newDiam? : number) {
        ...
        //Assign new coordinates for the center of the bounding box
        this.x = newCentX;
        this.y = newCentY;
        
        //Only assign a new diameter and radius if newDiam is not undefined
        this.diam = newDiam === undefined ? this.diam : newDiam;
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        ...
        //Only draw the target if it is visible
        if(this.visible)
        {
            //Create a new circle path
            let circle = new Path2D;

            //Define the circle to be filled
            circle.arc(this.centerX, this.centerY, this.radius, 0, 2 * Math.PI);

            //Choose a fill color and fill the arc
            ...
            //Start, define, and close the circle's outline
            ctx.beginPath();
    ...
    override pickedBy(ptX : number, ptY : number) : boolean {
        ...
        // === YOUR CODE HERE ===
        super.pickedBy(ptX, ptY);

        //If the click is inside the circle, then return true, else return false
        //The condition of the if statement calculates whether the distance from the center of the circle 
        //to the point clicked is less than the radius
        if(Math.sqrt(Math.pow(Math.abs(ptX - this.centerX), 2) + Math.pow(Math.abs(ptY - this.centerY), 2)) <= this.radius)
        {
            return true;
        }
        return false;
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        ...
        // === YOUR CODE HERE ===
        super.handleClickAt(ptX, ptY);

        //If we are in the in-trial state, the click is inside the circle, and the target is visible,
        //then return true and set up the next trial, else return false
        if(this.parentUI.currentState === "in_trial" && this.pickedBy(ptX, ptY) && this.visible)
        {
            //Record the trial end, set up a new trial, and tell the program to redraw the target
...
class Reticle extends Target {
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        ...
        if(this.visible)
        {
            //Create a new path for the crosshair
            let crossHair = new Path2D;

            //Define the circle to be filled
            crossHair.arc(this.centerX, this.centerY, Reticle.RETICLE_DIAM/2, 0, 2 * Math.PI);
            crossHair.closePath
            ...
            //Set the color of the reticle's outline
            ctx.strokeStyle = 'black';

            //Start the reticle's outline
            ctx.beginPath();
    ...
    override pickedBy(ptX : number, ptY : number) : boolean {
        ...
        //If the click is inside the circle, return true, else return false
        //The condition of the if statement calculates whether the distance from the center of the circle 
        //to the point clicked is less than the inner radius
        let distance: number = Math.sqrt(Math.pow(Math.abs(ptX - this.centerX), 2) + Math.pow(Math.abs(ptY - this.centerY), 2));
        if(distance <= Reticle.RETICLE_INNER_DIAM/2)
        {
            return true;
        }
        return false;
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        ...
        //If we are in the begin-trial state, the click is inside the inner circle, and the target is visible,
        //then return true and start the trial, else return false
        if(this.parentUI.currentState === 'begin_trial' && this.pickedBy(ptX, ptY) && this.visible)
        {
            this.parentUI.startTrial(ptX, ptY);
            this.parentUI.configure('in-trial');
            return true;
        }
        return false;
    }
...
class BackgroundDisplay extends ScreenObject{
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        ...
        // === YOUR CODE HERE ===
        //If the state is the start, then start a new trial
